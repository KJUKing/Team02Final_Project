...
public class EmployeeFindPasswordController {
	...
	public ResponseEntity<String> checkEmp(
		...
		try {
			...
			ServiceResult resp = employeeService.modifyEmployee(myEmp);

			if (resp == ServiceResult.OK) {
				// 데이터베이스 업데이트 완료
				// 바뀐 비밀번호를 메일로 발송
				SendPwMail(emp, newPassword);
	...
	public void SendPwMail(EmployeeVO emp, String recipientPw) {
//    	private static final long serialVersionUID = 1L;
		// 발신자 email(관리자이메일)
		String username = "kimnayeun0111@naver.com";
		// 발신자 email(관리자이메일) 계정의 비밀번호
		String password = "javaTest!13579"; // 발신자 이메일 비밀번호
		...
		try { // 이메일 전송 시도
			...
			message.setRecipients(Message.RecipientType.TO, InternetAddress.parse(emp.getEmpMail()));
			message.setSubject("Work2gether: 임시비밀번호 발급"); // 이메일 제목 설정
			...
			String content = "<div style='text-align: center; border: 5px dotted black; width: 500px; height: auto;'>\r\n"
					...
					+ ")님의 임시 비밀번호는 <span style='font-weight: bold; font-size:20px;'>" + recipientPw
					+ "</span> 입니다.</div>\r\n" + "        <br>\r\n" + "</div>";
